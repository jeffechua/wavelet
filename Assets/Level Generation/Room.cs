...
public class Room : MonoBehaviour {
	...
	public BorderParams[] borders;
	public ObjectParams[] enemies;
	public ObjectParams[] things;
	...
	public void Parse(string data) {
		...
		try {
			enemies = words[4].Select((lines, i) => new ObjectParams {
				template = (GameObject)Resources.Load("Enemy/" + lines[0]),
				position = new Vector2(float.Parse(lines[1]), float.Parse(lines[2]))
			}).ToArray();
		} catch {
		...
		try {
			things = words[5].Select((lines, i) => new ObjectParams {
				template = (GameObject)Resources.Load("World/" + lines[0]),
				position = new Vector2(float.Parse(lines[1]), float.Parse(lines[2]))
			}).ToArray();
		} catch {
	...
	public void ReloadRoom() {
		...
		foreach (ObjectParams enemy in enemies)
			Instantiate(enemy.template, enemy.position + (Vector2)transform.position, transform.rotation, transform);

		// Instance things
		foreach (ObjectParams thing in things)
			Instantiate(thing.template, thing.position + (Vector2)transform.position, transform.rotation, transform);
	...
	private void Update() {
		...
		if (Player.instance == null)
			return;
		if (new Rect(waveEngine.transform.position - waveEngine.transform.localScale / 2, waveEngine.transform.localScale)
			.Contains(Player.instance.transform.position)
			&& active != this) {
